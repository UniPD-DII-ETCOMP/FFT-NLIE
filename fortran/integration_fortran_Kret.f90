...
! mwPointer stuff for mexing
	  ...
	  mwPointer K1_re_pr, K2_re_pr, K3_re_pr
	  mwPointer K1_im_pr, K2_im_pr, K3_im_pr
	  mwPointer freq_pr
      mwPointer m, n, s
...
! fortran subroutine arguments
	  ...
	  real*8,allocatable,dimension(:) :: K1_re, K2_re, K3_re
	  real*8,allocatable,dimension(:) :: K1_im, K2_im, K3_im
	  real*8 L_re, MM_re, N_re, Npx_re, N_thread_re
	  real*8 Npy_re, Npz_re, freq
...
!     Check that input #15and fetch it
	  ...
	  if(debu) write(66,*) 'input15 checked N_thread = ', N_thread		  
...
!     Check that input #22and fetch it
      m = mxGetM(prhs(22))
      n = mxGetN(prhs(22))
      if(m .ne. 1 .or. n .ne. 1) then
         call mexErrMsgIdAndTxt ('MATLAB:convec:NonRowVector', &
                                'Input 22 must be freq 1x1.')
      endif
      siz = m*n
      freq_pr = mxGetPr(prhs(22))
      call mxCopyPtrToReal8(freq_pr, freq, siz)
	  if(debu) write(66,*) 'input22 checked freq = ', freq	  
	  
! call the computational subroutine.
      allocate(K1_re(L*MM*NN))
	  allocate(K2_re(L*MM*NN))
	  allocate(K3_re(L*MM*NN))
	  allocate(K1_im(L*MM*NN))
	  allocate(K2_im(L*MM*NN))
	  allocate(K3_im(L*MM*NN))
	  if(debu) write(66,*) 'chiamo la subroutine'
      call computeGREEN_f90_ret(L,MM,NN,x_src,Npx,face,r_f,r_e,u_e,triang_face,idxf_dot,&
                            idxf_cross,wx,d,N_thread,Npy,Npz,wy,wz,y_src,z_src,freq, & 
							K1_re,K2_re,K3_re,K1_im,K2_im,K3_im)
...
! Load the output into a MATLAB array.
      K1_re_pr = mxGetPr(plhs(1))
      siz=mo*no
      call mxCopyReal8ToPtr(K1_re, K1_re_pr, siz)
      if(debu) write(66,*) 'copied K1_re'  
	  deallocate(K1_re)
...
! Load the output into a MATLAB array.
      K2_re_pr = mxGetPr(plhs(2))
      siz=mo*no
      call mxCopyReal8ToPtr(K2_re, K2_re_pr, siz)
      if(debu) write(66,*) 'copied K2_re'  
	  deallocate(K2_re)
...
! Load the output into a MATLAB array.
      K3_re_pr = mxGetPr(plhs(3))
      siz=mo*no
      call mxCopyReal8ToPtr(K3_re, K3_re_pr, siz)
      if(debu) write(66,*) 'copied K3_re'  
	  deallocate(K3_re)
...
! Load the output into a MATLAB array.
      K1_im_pr = mxGetPr(plhs(4))
      siz=mo*no
      call mxCopyReal8ToPtr(K1_im, K1_im_pr, siz)
      if(debu) write(66,*) 'copied K1_im'  
	  deallocate(K1_im)
...
! Create a matrix for the return argument
      ...
      no=1
	  ComplexFlag = 0
      plhs(5) = mxCreateDoubleMatrix(mo, no, ComplexFlag)
! Load the output into a MATLAB array.
      K2_im_pr = mxGetPr(plhs(5))
      siz=mo*no
      call mxCopyReal8ToPtr(K2_im, K2_im_pr, siz)
      if(debu) write(66,*) 'copied K2_im'  
	  deallocate(K2_im)
...
! Create a matrix for the return argument
      ...
      no=1
	  ComplexFlag = 0
      plhs(6) = mxCreateDoubleMatrix(mo, no, ComplexFlag)
! Load the output into a MATLAB array.
      K3_im_pr = mxGetPr(plhs(6))
      siz=mo*no
      call mxCopyReal8ToPtr(K3_im, K3_im_pr, siz)
      if(debu) write(66,*) 'copied K3_im'  
	  deallocate(K3_im)	  
...
subroutine computeGREEN_f90_ret(L,M,N,x_src,Npx,face,r_f,r_e,u_e,triang_face,idxf_dot,&
                            idxf_cross,wx,d,N_thread,Npy,Npz,wy,wz,y_src,z_src,freq,&
							K1_re,K2_re,K3_re,K1_im,K2_im,K3_im)
integer*8 L,M,N,Npx
integer*8 Npy, Npz
...
real*8 res1_re, res2_re, res1_im, res2_im
real*8 wy(Npy),wz(Npz)
real*8 K1_re(L*M*N), mm(3)
real*8 K2_re(L*M*N), K3_re(L*M*N)
real*8 K1_im(L*M*N), K2_im(L*M*N), K3_im(L*M*N)
real*8 dx, dy, dz
real*8 freq, pi, k0

pi=4.0d0*atan(1.0d0)

k0 = 2.d0*pi*freq/299792458.d0
...
!$OMP      triang_face,idxf_dot,idxf_cross,wx,wy,wz,K1_re,K2_re,K3_re,K1_im,K2_im,K3_im,y_src,z_src, & 
!$OMP      dx,dy,dz,k0)
!$OMP DO SCHEDULE(DYNAMIC,1) PRIVATE(mx,my,mz,mm,ee,r_m,x_trg,y_trg,y_trg_m,z_trg,res1,res2,res1_re,res2_re,res1_im,res2_im)
do mx = 1,L
        do my = 1,M
            do mz = 1,N
                mm(1) = real(mx,8)
				...
				do ee = 1,Npy
					...
					y_trg_m(1) = y_trg_m(1)+y_trg(ee,1)
					y_trg_m(2) = y_trg_m(2)+y_trg(ee,2)
					y_trg_m(3) = y_trg_m(3)+y_trg(ee,3)
				enddo
				y_trg_m(1:3)=y_trg_m(1:3)/real(Npy,8)
				
				call Integrate_NumAn3(face,r_f,r_e,u_e,triang_face,idxf_dot,&
				         idxf_cross,y_trg,wy,Npy,res1)		
				res1_re = res1*cos(-k0*(y_trg_m(1)**2.d0+y_trg_m(2)**2.d0+y_trg_m(3)**2.d0)**0.5)
				res1_im = res1*sin(-k0*(y_trg_m(1)**2.d0+y_trg_m(2)**2.d0+y_trg_m(3)**2.d0)**0.5)
                ...
                do ee = 1,Npy
					...
					y_trg_m(3)=y_trg_m(3)+y_trg(ee,3)
				enddo
				y_trg_m(3)=y_trg_m(3)/real(Npy,8)
				call Integrate_NumAn3(face,r_f,r_e,u_e,triang_face,idxf_dot,&
				         idxf_cross,y_trg,wy,Npy,res2)	
				res2_re = res2*cos(-k0*(y_trg_m(1)**2.d0+y_trg_m(2)**2.d0+y_trg_m(3)**2.d0)**0.5)
				res2_im = res2*sin(-k0*(y_trg_m(1)**2.d0+y_trg_m(2)**2.d0+y_trg_m(3)**2.d0)**0.5)		 
                K3_re(mx+(my-1)*L+(mz-1)*L*M) =  ((-res1_re+res2_re)*(1.d0/(dx*dz))*(dx/(dy*dz)))
				K3_im(mx+(my-1)*L+(mz-1)*L*M) =  ((-res1_im+res2_im)*(1.d0/(dx*dz))*(dx/(dy*dz)))
				...
				do ee = 1,Npz
					...
					y_trg_m(1) = y_trg_m(1)+z_trg(ee,1)
					y_trg_m(2) = y_trg_m(2)+z_trg(ee,2)
					y_trg_m(3) = y_trg_m(3)+z_trg(ee,3)
				enddo
				y_trg_m(1:3)=y_trg_m(1:3)/real(Npz,8)
                ...
                do ee = 1,Npz
					...
					y_trg_m(2) = y_trg_m(2)+z_trg(ee,2)
				enddo
				y_trg_m(2)=y_trg_m(2)/real(Npz,8)				
				call Integrate_NumAn3(face,r_f,r_e,u_e,triang_face,idxf_dot,&
				         idxf_cross,z_trg,wz,Npz,res2)			
				res2_re = res2*cos(-k0*(y_trg_m(1)**2.d0+y_trg_m(2)**2.d0+y_trg_m(3)**2.d0)**0.5)
				res2_im = res2*sin(-k0*(y_trg_m(1)**2.d0+y_trg_m(2)**2.d0+y_trg_m(3)**2.d0)**0.5)         						 
                K2_re(mx+(my-1)*L+(mz-1)*L*M) =  -((res1_re-res2_re)*(1.d0/(dx*dy))*(dx/(dy*dz)))	
                K2_im(mx+(my-1)*L+(mz-1)*L*M) =  -((res1_im-res2_im)*(1.d0/(dx*dy))*(dx/(dy*dz)))					
                ...
                do ee = 1,Npz
					...
					y_trg_m(1)=y_trg_m(1)+z_trg(ee,1)
				enddo
				y_trg_m(1)=y_trg_m(1)/real(Npz,8)				
				call Integrate_NumAn3(face,r_f,r_e,u_e,triang_face,idxf_dot,&
				         idxf_cross,z_trg,wz,Npz,res2)	
				res2_re = res2*cos(-k0*(y_trg_m(1)**2.d0+y_trg_m(2)**2.d0+y_trg_m(3)**2.d0)**0.5)
				res2_im = res2*sin(-k0*(y_trg_m(1)**2.d0+y_trg_m(2)**2.d0+y_trg_m(3)**2.d0)**0.5)								
                K1_re(mx+(my-1)*L+(mz-1)*L*M) =  ((-res1_re+res2_re)*(1.d0/(dx*dy))*(dy/(dx*dz)))				
				K1_im(mx+(my-1)*L+(mz-1)*L*M) =  ((-res1_im+res2_im)*(1.d0/(dx*dy))*(dy/(dx*dz)))
            enddo
        enddo
enddo
!$OMP END DO NOWAIT 
...
end subroutine computeGREEN_f90_ret
