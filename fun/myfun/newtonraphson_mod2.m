function [x, resnorm, F, exitflag, output, jacob,resnormstore,xstore] = newtonraphson_mod2(fun, x0, options)
...
% fun = fun.F(x)  valuta la funzione A(x)*x-b in x dove A è il sistema di
%                   equazioni e b è la rhs
%       fun.solvJ(x,FF)   risolve il sistema J(x)\b dove J è lo jacobiano 
%       fun.JTFF_T(x,FF)   effettua la seguente operazione:  (J(x)^T*FF)^T  
%% initialize
...
resnormstore=zeros(MAXITER,1);
xstore=zeros(length(x0),MAXITER);
...
while (resnorm>TOLFUN || lambda<1) && exitflag>=0 && Niter<=MAXITER
    resnormstore(Niter+1)=resnorm;
    figure(251292)
    semilogy(resnormstore,'-x')
    title('Newton Rhapson')
    drawnow
    if lambda==1
        %% Newton-Raphson solver
        Niter = Niter+1; % increment counter
        dx_star=-(fun.solvJ(x,F));
%         dx_star = -J\F; % calculate Newton step
        % NOTE: use isnan(f) || isinf(f) instead of STPMAX
        dx = dx_star.*TYPX; % rescale x
        g=fun.JTFF_T(x,F);
%         g = F'*J; % gradient of resnorm F'*J=(J'*F)'
...
jacob = 'not possible in this version';
end
...
try
    [F, J] = fun(x);
%     warning('with user jacobian'); 
catch 
    F = fun(x);
    J = jacobian(fun, x); % evaluate center diff if no Jacobian
%     warning('evaluate center diff if no Jacobian'); 
end
